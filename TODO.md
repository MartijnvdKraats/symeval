...
## Package
- Two bugs in `quantity_evalf`/`sym_evalf` surfaced while building the simply supported beam example (see below): [#4](https://github.com/bedrock-engineer/symeval/issues/4) `Piecewise` can't be evaluated directly (relational conditions can't be decided once substituted values carry unit-symbol factors), [#5](https://github.com/bedrock-engineer/symeval/issues/5) an exactly-zero result loses its unit and raises `DimensionalityError`. Both open, no fix merged yet. The example works around both at the notebook level (workarounds are comment-tagged `symeval issue #4` / `symeval issue #5` in `examples/simply_supported_beam.py` for easy discovery) — once fixed upstream, revisit those spots and simplify.
...
- ~~Simply Supported Beam with Linearly Distributed Load~~: https://imartincei.github.io/CalcpadCE/examples/simply-supported-beams.html — done: `examples/simply_supported_beam.py` covers all six load cases on that reference page (uniform, linear, partial, concentrated force, concentrated forces, concentrated moment), not just the linear-load one. Docs integration (`docs/examples/simply-supported-beam.qmd`, sidebar entry) also done. Not yet opened as a real PR — sitting as a draft on `MartijnvdKraats/symeval#example_notebook_1`, blocked on the two upstream bugs above (#4, #5); client to weigh in on timing before it goes up for review.
...
- `mo.image(..., caption=...)` throws a React error (#62: "the `style` prop expects a mapping... not a string") in the quarto-marimo islands runtime specifically — traced it to marimo's `image.py` passing raw string `style="..."` attributes (`h.figure(..., style="display: flex...")`, `h.img(..., style=styles)`), which the islands' React hydration apparently can't handle even though the same code renders fine in the native marimo web app and in `docs_render`'s server-side execution. Worked around in `examples/simply_supported_beam.py` by dropping `mo.image()` entirely for the six beam schematics and using plain markdown `![]()` syntax instead (routes through quarto/pandoc's own `<img>` rendering, never touches the marimo image widget) plus marimo's `public/` convention (`examples/public/`, mirrored to `docs/examples/public/` by `scripts/examples_to_qmd.py`) for the local-asset resolution `mo.image()` would otherwise have handled. Worth a minimal repro + upstream issue if `mo.image()` + `caption` is needed again.
